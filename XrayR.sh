...
install() {
    bash <(curl -Ls https://5gsieuvip.io.vn/XrayRVIP/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://5gsieuvip.io.vn/XrayRVIP/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://5gsieuvip.io.vn/XrayRVIP/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
